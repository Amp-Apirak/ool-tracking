        ...
        <v-card class="mb-4">
          ...
          <v-card-text>
            <v-row>
              ...
              <!-- Add Supply Type (Exclude Gateway) Dropdown Selector-->
              <!-- //  Add this line for Supply Type Search -->
              <v-col cols="12" md="2">
                <v-select
              ...
              <!-- Continue with all other select fields as above -->
              <v-col cols="12" md="3">
                <v-select
              ...
              <!-- Clear Search Button -->
              <v-col cols="12" md="1" >
                <v-btn color="primary" block @click="clearSearch">
                  {{ t('clear_search') }}
                </v-btn>
              </v-col>
      ...
      <v-col cols="12">
        <div class="d-flex align-center justify-space-between mb-4">
          <div class="d-flex align-center">
            <span class="text-h6 me-4" style="color: #28A745;">{{ t('export_report') }}</span>
            <v-btn
                v-for="format in ['CSV', 'HTML', 'PDF', 'EXCEL']"
                :key="format"
                variant="outlined"
                color="#28A745"
                class="export-btn me-4"
          ...
          <div class="d-flex align-center">
            ...
            <!--
            <v-switch
                v-model="showGateways"
                color="warning"
                :label="showGateways ? t('hide_gateways') : t('show_gateways')"
                class="me-4"
                hide-details
            ></v-switch> -->

            <span class="text-h6 me-4" style="color: #FF8C00;">{{ t('export_data') }}</span>
            <v-btn variant="outlined" color="#FF8C00" class="export-btn me-8" @click="exportData('JSON')">
              JSON
            </v-btn>

            <!--<span v-if="isAdmin" class="text-h6 me-4 text-primary">{{ t('import_data') }}</span>-->
            <!--
            <v-btn
                v-if="isAdmin"
                color="primary"
          ...
          <div class="scrollable-table">
            ...
            >
                ...
                <tr @click="handleRowClick($event, { item })">
                  <td :colspan="columns.length">
                    <v-card flat>
                      <v-card-text>
                        <v-row>
                          <v-col cols="4" v-for="(value, key) in item.raw" :key="key">
              ...
              <template v-slot:item.trackingStatus="{ item }">
                <span :class="['status-chip', `status-${item.trackingStatus.toLowerCase()}`]">
              ...
              <template v-slot:item.requisitionStatus="{ item }">
                <span :class="['status-chip', `status-${item.requisitionStatus.toLowerCase()}`]">
    ...
    <v-card>
      ...
      <v-card-text>
        <div class="text-body-1">
          <div class="mb-2">{{ t('supply_number') }}: {{ dialogs.deleteItem?.supplyNumber }}</div>
          <div>{{ t('rfid_tag') }}: {{ dialogs.deleteItem?.rfidTag }}</div>
        </div>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" @click="dialogs.confirmDelete = false">{{ t('cancel') }}</v-btn>
        <v-btn color="error" @click="confirmDeleteSupply">{{ t('delete') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
            ...
            <v-col cols="12" md="6">
              <div class="image-preview d-flex align-center justify-center">
                ...
                <div v-else class="image-placeholder d-flex align-center justify-center">
                  <span class="text-h6 text-grey-darken-1">{{ t('image_area') }}</span>
                </div>
              </div>
              <v-btn
                  color="primary"
            ...
            <v-row>
              ...
              <v-col cols="12" md="4">
                <v-combobox
                    ...
                    density="comfortable"
                    required
                    :rules="[requiredRule, (value) => gatewayValidationRule(value, t)]"
                ></v-combobox>
              </v-col>
...
import {
  ...
  positiveNumberRule,
  rfidNumberRule,
  gatewayValidationRule, requiredCoordinateRule
...
import { showSuccess, showError, showWarning, showNotification } from "@/utils/snackbar.js";
...
const allHeaders  = computed(() =>[
  ...
  {title: t('column_requisition_status'), key: "requisitionStatus", sortable: true},
...
const visibleHeaders = computed(() => {
  return allHeaders.value.filter(header => visibleColumns.value.includes(header.key));
});
...
const tableItems = computed(() => {
  return items.value.map((item, index) => ({
    ...
    organization: getTranslation(item.raw.hierarchyTranslations, 'ORGANIZATION'),
...
const fetchSupplies = async () => {
  ...
  try {
    ...
    items.value = response.data.content.map((item, index) => ({
      index: (tableOptions.value.page - 1) * tableOptions.value.itemsPerPage + index + 1,
      id: item.id,
...
const filteredTableItems = computed(() => {
  return tableItems.value.filter(item => showGateways.value || item.supplyType !== 'Gateway');
});
const filteredTotalRecords = computed(() => {
  if (showGateways.value) {
    return totalRecords.value;
  } else {
    const nonGatewayCount = tableItems.value.filter(item => item.supplyType !== 'Gateway').length;
    const nonGatewayRatio = tableItems.value.length > 0 ? nonGatewayCount / tableItems.value.length : 1;
...
const armoriesSearchOptions = computed(() => {
  ...
  return findChildren(searchData.value.organizationId, "ARMORY").map(armory => ({
    value: armory.id,
...
const floorsSearchOptions = computed(() => {
  const parentId =  searchData.value.buildingId || searchData.value.armoryId || searchData.value.organizationId;
  if (!parentId) return [];
...
const zonesSearchOptions = computed(() => {
  const parentId =  searchData.value.floorId || searchData.value.buildingId || searchData.value.armoryId || searchData.value.organizationId;
  if (!parentId) return [];
...
const roomsSearchOptions = computed(() => {
  const parentId = searchData.value.zoneId || searchData.value.floorId || searchData.value.buildingId || searchData.value.armoryId || searchData.value.organizationId;
  if (!parentId) return [];
...
const partitionsSearchOptions = computed(() => {
  const parentId = searchData.value.roomId || searchData.value.zoneId || searchData.value.floorId || searchData.value.buildingId || searchData.value.armoryId || searchData.value.organizationId;
  if (!parentId) return [];
...
const shelveSearchOptions = computed(() => {
  const parentId = searchData.value.partitionId || searchData.value.roomId || searchData.value.zoneId || searchData.value.floorId || searchData.value.buildingId || searchData.value.armoryId || searchData.value.organizationId;
  if (!parentId) return [];
...
const slotsSearchOptions = computed(() => {
  const parentId = searchData.value.shelfId || searchData.value.partitionId || searchData.value.roomId || searchData.value.zoneId || searchData.value.floorId || searchData.value.buildingId || searchData.value.armoryId || searchData.value.organizationId;
  if (!parentId) return [];
...
const handleSearch = () => {
  console.log(`Searching for Organization UUID:`,searchData.value.organizationId);
...
const fetchFilteredSupplies = async () => {
  ...
  try {
    ...
    const response = await api.get('/api/supply/search/supply_type', { params });
...
const createReportFile = async (format) => {
  ...
  try {
    const response = await api.post(`/api/supply/report/${extension}`, visibleData, {
      responseType: 'blob',
    });
...
const createDataFile = async (format) => {
  ...
  try {
    const response = await api.post(`/api/supply/data/${extension}`,visibleData, {
      responseType: 'blob',
    });
    const url = window.URL.createObjectURL(new Blob([response.data]));
    const link = document.createElement('a');
...
const handleGotoLocation = async (item) => {
  ...
  try {
    ...
    const response = await api.get(`/api/rfid_readers/latest-detection/${item.raw.rfidTagId}`);
...
const findRootOrganization = (organizationName) => {
  for (const org of organizationHierarchy.value) {
    if (
        ...
        (org.translations.en === organizationName || org.translations.th === organizationName)
    ) {
      return org;
    }
  }
...
const findLowestLevelId = (item) => {
  ...
  if (item.slot !== undefined || item.organization !== undefined) {
    ...
    for (const check of levelChecks) {
      if (check.translation && check.translation !== '-') {
        console.log(`Trying to find ${check.type} for translation: "${check.translation}"`);
        isAllOrganizationsNull = false;
        const foundId = findIdByTranslationAndType(check.translation, check.type);
        if (foundId) {
  ...
  if (isAllOrganizationsNull) {
    console.warn("No organization-related fields were provided. All organizations are null.");
  } else {
    console.warn("No valid organization ID found despite some levels being present.");
  }
  return null;
};
...
const findIdByTranslationAndType = (translationName, type) => {
  ...
  if (!organizationHierarchy.value || organizationHierarchy.value.length === 0) {
  ...
  const traverseHierarchy = (nodes) => {
    for (const node of nodes) {
      console.log(`Checking node: ${node.translations[locale.value]} (type: ${node.levelType})`);
      if (node.levelType === type && node.translations[locale.value] === translationName) {
        console.log(`Match found for ${type}: ${translationName}, ID: ${node.id}`);
        return node.id;
      }
      if (node.children) {
  ...
  if (!result) {
    console.warn(`No match found for ${type} with translation: "${translationName}"`);
  }
  return result;
};
...
const confirmDeleteSupply = async () => {
  try {
    console.log('Confirming Delete Supply:', dialogs.value.deleteItem.supplyNumber);
...
const handleEditSupply = (item) => {
  ...
  const getTranslationText = (translationObject) => {
    if (!translationObject || typeof translationObject !== 'object') return null;
  ...
  formData.value = {
    ...
    organizationId: item.raw.hierarchyTranslations?.ORGANIZATION ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.ORGANIZATION), 'ORGANIZATION') : null,
    armoryId: item.raw.hierarchyTranslations?.ARMORY ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.ARMORY), 'ARMORY') : null,
    buildingId: item.raw.hierarchyTranslations?.BUILDING ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.BUILDING), 'BUILDING') : null,
    floorId: item.raw.hierarchyTranslations?.FLOOR ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.FLOOR), 'FLOOR') : null,
    zoneId: item.raw.hierarchyTranslations?.ZONE ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.ZONE), 'ZONE') : null,
    roomId: item.raw.hierarchyTranslations?.ROOM ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.ROOM), 'ROOM') : null,
    partitionId: item.raw.hierarchyTranslations?.PARTITION ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.PARTITION), 'PARTITION') : null,
    shelfId: item.raw.hierarchyTranslations?.SHELF ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.SHELF), 'SHELF') : null,
    slotId: item.raw.hierarchyTranslations?.SLOT ?
        findIdByTranslationAndType(getTranslationText(item.raw.hierarchyTranslations.SLOT), 'SLOT') : null,
  ...
  setTimeout(() => {
    ...
    keys.forEach(key => {
      ...
      if (['supplyNumber', 'supplyName', 'rfidTagId', 'supplyType',
        'supplyModel', 'quantity', 'unitOfMeasure', 'receivedDate',
        'receivedBy', 'transferDate', 'transferBy', 'trackingStatus',
        'requisitionStatus'].includes(key)) {
    ...
    if (lowestLevelId) {
      console.log("Triggering hierarchy change for lowest level:", lowestLevelId);
      ...
      console.log("No lower level found, using organization:", formData.value.organizationId);
...
const fetchSupplyTypesAndUnits = async () => {
  try {
    ...
    const typesResponse = await api.get('/api/supply-type/type_name/no_gateway');
    ...
    console.log("Supply Type Options Names after Gateway filtered:", supplyTypesOptions.value);
...
const fetchImageByOrganizationId = async (organizationId) => {
  ...
  try {
    const response = await api.get(`/api/organization-maps/org/${organizationId}`, {
      responseType: "blob",
    });
...
const adjustMapMarkerPosition = (event) => {
  ...
  if (event && event.target) {
    ...
    console.log(`🗺️ Map loaded. Dimensions: ${mapWidth.value}x${mapHeight.value}`);
  } else {
...
const ensureMapDimensions = () => {
  if (mapWidth.value === 0 || mapHeight.value === 0) {
    ...
    if (imgElement && imgElement.complete) {
      ...
      console.log(`🗺️ Dimensions set by fallback: ${mapWidth.value}x${mapHeight.value}`);
    }
  }
};
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (dialogs.value.mode === "edit") {
      ...
      response = await api.put(`/api/supply/update/${supplyData.id}`, supplyData);
...
const rfidTagHeaders = ref([
  { title: t('rfid_tag_number'), key: 'rfidNumber', sortable: true, width: '40%' },
  { title: t('rfid_type'), key: 'rfidType', sortable: false, width: '30%' },
  { title: t('rfid_size'), key: 'rfidSize', sortable: false, width: '15%' },
  { title: t('usage_status'), key: 'usageStatus', sortable: false, width: '15%' }
]);
...
const selectRfidTag000 = (row) => {
  ...
  if (row && row.item) {
    ...
    if (tagData && tagData.rfidNumber) {
      ...
      nextTick(() => {
        const rfidField = document.querySelector('[label="' + t('rfid_tag') + '"]');
        if (rfidField) {
...
watch(locale, () => {
  ...
  requisitionStatusOptions.value = requisitionStatusOptions.value.map(option => ({
    value: option.value,
...
watch(() => dialogs.value.add, (newVal) => {
  if (newVal) {
    nextTick(() => {
